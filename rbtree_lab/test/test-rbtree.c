...
// The values of ch[1] subtree should be greater than or equal to the current
// node
                            ...
                            node_t *nil)
{
  ...
  *min = *max = p->key;
  ...
  const bool lr = search_traverse(p->ch[0], &l_min, &l_max, nil);
  if (!lr || l_max > p->key)
  {
    return false;
  }
  const bool rr = search_traverse(p->ch[1], &r_min, &r_max, nil);
  if (!rr || r_min < p->key)
  {
    return false;
  }
                           ...
                           const int black_depth, node_t *nil)
{
  ...
  return color_traverse(p->ch[0], p->color, next_depth, nil) &&
         color_traverse(p->ch[1], p->color, next_depth, nil);
}
